...
namespace DynamicOdata.Service
{
    public class SqlQueryBuilder
    {
        ...
        private string BuildSingleClause(SingleValuePropertyAccessNode propertyNode, ConstantNode valueNode, BinaryOperatorKind operatorKind)
        {
            ...
            var value = propertyNode.Property.Name;

            var isNumber = valueString.All(char.IsNumber);

            if (!isNumber && 
                (propertyNode.Property.Type.Definition.ToString().ToLower().Contains("string") ||
                (propertyNode.Property.Type.Definition.ToString().ToLower().Contains("bool"))))
